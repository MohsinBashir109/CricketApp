...
import TournamentRow, { TournamentRowProps } from './TournamentRow';
import { fontFamilies } from '../../utils/fontfamilies';
import { fontPixel, heightPixel, widthPixel } from '../../utils/constants';
import { cardShadowSm } from '../../utils/cardShadow';

type ActiveTournamentsCardProps = {
  title: string;
  subtitle: string;
  headerIcon: any;
  ...
  isDark?: boolean;
  rows: (TournamentRowProps & {
    id: string;
    borderColor: string;
    backgroundColor: string;
    /** When true, row uses `tornamentcard` art (e.g. active preview on Tournaments home). */
  ...
  return (
    <View style={styles.card}>
        ...
        >
          {dropdownItems.map(item => (
            <Pressable
              ...
              style={({ pressed }) => [styles.dropdownItem, pressed ? styles.dropdownItemPressed : null]}
            >
      ...
      <View style={styles.rowsWrap}>
        {rows.map(r => {
          const { id, borderColor, backgroundColor, artBackground, ...rowProps } = r;
          return (
            <View
              key={id}
              style={[
                ...
                {
                  borderColor,
                  backgroundColor: artBackground ? 'transparent' : backgroundColor,
                  overflow: artBackground ? 'hidden' : 'visible',
                },
              ]}
            >
              <TournamentRow {...rowProps} artBackground={artBackground} />
            </View>
          );
        })}
      </View>
      ...
      <Pressable
        ...
        style={[
          styles.viewAllRow,
          { backgroundColor: viewAllBg, borderColor: viewAllBorder },
          viewAllDisabled ? styles.disabled : null,
        ]}
      >
        <ThemeText color="primary" style={styles.viewAllText}>
          View all active tournaments
        </ThemeText>
        <ThemeText color="primary" style={styles.viewAllArrow}>
          →
        </ThemeText>
      </Pressable>
    </View>
...
export default ActiveTournamentsCard;
